...
import { StyleSheet, View, Text, Alert, TextInput, KeyboardAvoidingView, ActivityIndicator, TouchableOpacity, Image, Platform, WebView } from 'react-native';
...
export default class index extends Component {
  constructor(props) {
    ...
    this.state = { chargedAmount: 0, cardno: '', cvv: '', status: "", chargeResponseMessage: '', suggested_auth: "", vbvModal: false, vbvurl: '', cardnoErr: 'none', cardErr: '#fff', dateErr: 'none', cvvErr: 'none', expirymonth: '', expiryyear: '', firstname: 'Oluwole', lastname: 'Adebiyi', email: 'flamekeed@gmail.com', pin: "", pinModal: false, otp: "", flwRef: "", otpModal: false, intlModal: false, loading: false, otp: "", intl: {}, checked: false, address: '', city: '', state: '', zipcode: '', country: '', addressErr: 'none', cityErr: 'none', stateErr: 'none', zipcodeErr: 'none', countryErr: 'none', inputErr: '#fff' };
  ...
  confirmVBV(err, data) {
    ...
    if (data.status == "successful") {
      this.props.rave.verifyTransaction(data.txRef).then((resp) => {
        this.setState({ cardno: '', cvv: '', expirymonth: '', expiryyear: '', vbvModal: false })
        this.props.onSuccess(resp);
      }).catch((error) => {
        this.props.onFailure(error);
      })
  ...
  check() {
    this.setState({
      cardnoErr: 'none', dateErr: 'none', cvvErr: 'none', inputErr: '#fff'
    })
  ...
  _onNavigationStateChange(webViewState) {
    ...
    if (webViewState.url.includes("txRef")) {
      ...
      this.confirmVBV(false, JSON.parse(this.getParameterByName('response', webViewState.url)));
    }
  }
  ...
  checkIntl() {
    this.setState({
      addressErr: 'none', cityErr: 'none', stateErr: 'none', zipcodeErr: 'none', countryErr: 'none', inputErr: '#fff'
    })
  ...
  render() {
    ...
    if (this.state.pinModal) {
      page = <KeyboardAwareScrollView keyboardShouldPersistTaps="always">
        <View style={{ flex: 1 }}>
          <Text style={[styles.label, { fontSize: 20, marginVertical: 10 }]}>Please enter card pin to continue transaction</Text>
          <View style={styles.formGroup}>

            <View style={styles.input}>
              <Text style={{ color: '#999999', fontSize: 14 }}>Enter PIN</Text>
            ...
            <TouchableOpacity onPress={this.confirmPin} style={{ width: "100%", marginTop: 20 }} disabled={(this.state.loading == false) ? false : true}>
              <View style={{ backgroundColor: this.props.primarycolor, paddingVertical: 15, borderRadius: 5, marginTop: 20, opacity: (this.state.loading == false) ? 1 : 0.6 }}>
                <Text style={{ fontSize: 13, textAlign: "center", fontWeight: "bold" }}>Confirm PIN</Text>
              </View>
            </TouchableOpacity>
          </View>
        </View>
      ...
      if (this.state.otpModal) {
        page = <KeyboardAwareScrollView keyboardShouldPersistTaps='always'>
          <View style={{ flex: 1 }}>
            <View style={styles.formGroup}>
              <Text style={[styles.label, { fontSize: 20, marginVertical: 10 }]}>{this.state.chargeResponseMessage}</Text>
              <View style={styles.input}>
      ...
      page = <View style={{ flex: 1, width: '100%' }}>
        {web}
      </View>
      ...
      page = <KeyboardAwareScrollView keyboardShouldPersistTaps='always'>
        ...
        <View style={{ flex: 1, marginBottom: 50 }}>
          <View style={{ marginTop: 10 }}>
            <Text style={[styles.label, { fontSize: 18, marginVertical: 5 }]}>Please enter billing address details to continue transaction</Text>
            <View style={styles.input}>
          ...
          <TouchableOpacity onPress={this.submit} style={{ width: "100%", marginTop: 30 }} disabled={(this.state.loading == false) ? false : true}>
            <View style={{ backgroundColor: this.props.primarycolor, paddingVertical: 15, borderRadius: 5, opacity: (this.state.loading == false) ? 1 : 0.6 }}>
              <Text style={{ fontSize: 13, textAlign: "center", fontWeight: "bold", color: this.props.secondarycolor }}>ENTER</Text>
            </View>
          </TouchableOpacity>
        </View>
      </KeyboardAwareScrollView>
      ...
      page = <KeyboardAwareScrollView keyboardShouldPersistTaps='always'>
        ...
        <View style={{ flex: 1, height: 204 }}>
          <View style={styles.formGroup}>
            <Text style={[styles.label, { fontSize: 20, marginVertical: 10 }]}>Enter your card information to make payment</Text>
            <View style={[styles.input, { borderTopRightRadius: 3, borderTopStartRadius: 3 }]}>
              <Text style={{ color: '#999999', fontSize: 16 }}>CARD NUMBER</Text>
              <View style={{ paddingVertical: 10, flexDirection: 'row-reverse' }}>
                ...
                <View>
                  <TextInput
                    ...
                    keyboardType="numeric"
                    style={{ fontSize: 16, minWidth: "95%", paddingHorizontal: 10 }}
                    underlineColorAndroid='rgba(0,0,0,0)'
          ...
          <View style={styles.formGroup}>
            <View style={{ flexDirection: 'row', width: '100%' }}>
              <View style={{ flexGrow: 1, width: '50%', }}>
                <View style={[styles.input, { borderBottomLeftRadius: 3 }]}>
                  <Text style={{ color: '#999999', fontSize: 16 }}>VALID TILL</Text>
                  <View style={{ flexDirection: 'row' }}>
                    ...
                    <TextInput
                      ...
                      ref="1"
                      keyboardType="numeric"
                      style={{ fontSize: 16, flexGrow: 2, height: 45, textAlign: 'right', alignSelf: 'flex-start', width: '10%' }}
                      underlineColorAndroid='rgba(0,0,0,0)'
                      placeholder="MM"
                      maxLength={2}
                    ...
                    />
                    <Text style={{ fontSize: 16, paddingTop: 10, textAlign: 'center', alignSelf: 'stretch', width: '10%', color: '#999999' }}>/</Text>
                    <TextInput
                      autoCorrect={false}
                      editable={(this.state.loading) ? false : true}
                      ref="2"
                      keyboardType="numeric"
                      style={{ fontSize: 16, flexGrow: 2, height: 45, textAlign: 'left', alignSelf: 'flex-start', width: '50%' }}
                      underlineColorAndroid='rgba(0,0,0,0)'
                      placeholder="YY"
                      maxLength={2}
              ...
              <View style={{ flexGrow: 1, paddingLeft: 1, width: '50%' }}>
                <View style={[styles.input, { borderBottomRightRadius: 3 }]}>
                  <View style={{ fontSize: 16, flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={{ color: '#999999' }}>CVV/CVV2 </Text>
                    <Text style={{ color: '#999999', fontSize: 10, paddingVertical: 4.5 }}>What is this?</Text>
                  </View>
        ...
        <View style={{ color: '#4a4a4a', fontSize: 16, marginTop: 55, marginLeft: 0 }}>
